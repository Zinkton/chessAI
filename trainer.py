...
def generate_training_data_multiprocess(agent: ChessAgent, models, alpha):
    ...
    for x in range(constants.THREADS):
        ...
        opponent = agent_copy #model_utilities.copy_model(model_utilities.load_model(models, agent.name)) if x > 5 else agent_copy
        is_white = bool(x % 2)
...
def generate_training_data(input: GenerateTrainingDataInput):
    ...
    while outcome is None:
        ...
        position_score = position_score / max_value
        ...
        if outcome is None:
            ...
            if outcome is None:
                ...
                outcome_score = env.get_outcome_score(outcome) / max_value
...
def do_training():
    ...
    while True:
        ...
        for iteration in range(constants.ITERATIONS_PER_EPOCH):
            ...
            epoch_loss += iteration_loss / batch_count
            epoch_steps += iteration_steps
            print(f'Iteration {iteration}, Steps per second: {iteration_steps / (time.perf_counter() - iteration_start)}, Loss {iteration_loss / batch_count }')
